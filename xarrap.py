...
def find_accounts(phone_number):
    ...
    for platform, search_url in search_urls.items():
        ...
        if search_response.status_code == 200:
            print(f"\n{format_phone_number(phone_number)} Results from Google search on {platform}:")
...
def reverse_phone_lookup(phone_number):
    ...
    for platform, lookup_url in lookup_urls.items():
        ...
        if response.status_code == 200:
            print(f"\n{format_phone_number(phone_number)} {platform} results:")
...
def find_name_and_search(phone_number):
    ...
    if response.status_code == 200:
        print(f"\n{format_phone_number(phone_number)} Results from Google Dork:")
        soup = BeautifulSoup(response.text, 'html.parser')
        results = soup.find_all('div', {'class': 'yuRUbf'})
        ...
        for result in results:
            ...
            name_match = re.search(r'intitle:"name":(.*?)<', text)
            if name_match:
                extracted_name = name_match.group(1)
                print(f"\n{format_phone_number(phone_number)} Name found: {extracted_name}")

                search_url = f"https://www.google.com/search?q={extracted_name}"
                search_response = requests.get(search_url)

                if search_response.status_code == 200:
                    print(f"\n{format_phone_number(phone_number)} Additional results from Google search:")
                    soup = BeautifulSoup(search_response.text, 'html.parser')
                    results = soup.find_all('div', {'class': 'yuRUbf'})
